...
if YANDEX_API_KEY and YANDEX_FOLDER_ID:
    ...
    logger.info(
        "✅ Yandex Translator инициализирован (ключ: %s..., папка: %s)",
        YANDEX_API_KEY[:8],
        YANDEX_FOLDER_ID[:8],
    )
else:
...
def translate_to_russian(text: str) -> Optional[str]:
    ...
    try:
        ...
        payload = {"folderId": YANDEX_FOLDER_ID, "texts": [text], "targetLanguageCode": "ru"}
...
def translate_to_english(text: str) -> Optional[str]:
    ...
    try:
        url = "https://translate.api.cloud.yandex.net/translate/v2/translate"
        headers = {"Authorization": f"Api-Key {YANDEX_API_KEY}", "Content-Type": "application/json"}
        payload = {"folderId": YANDEX_FOLDER_ID, "texts": [text], "targetLanguageCode": "en"}
