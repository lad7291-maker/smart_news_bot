...
# === MEM-FIX: ранняя pidfile-проверка ДО тяжёлых импортов (torch/transformers ~1GB) ===
# systemd crash-loop: без этого каждый ложный рестарт грузит torch на 15-30 сек 100% CPU
_EARLY_PIDFILE = "/tmp/smart_news_bot.pid"
if os.path.exists(_EARLY_PIDFILE):
    try:
        with open(_EARLY_PIDFILE) as _f:
            _early_pid = int(_f.read().strip())
        os.kill(_early_pid, 0)
        print(f"🚫 Бот уже запущен (PID {_early_pid}). Завершение.", file=sys.stderr)
        sys.exit(0)
    except (ValueError, OSError, ProcessLookupError):
        pass  # stale pidfile — продолжаем, _check_pidfile() в main() перезапишет

# === MEM-FIX: glibc allocator — ограничить арены, агрессивнее возвращать память ОС ===
# image-pipeline в executor-тредах раздувает per-thread арены, которые не отдают RSS
try:
    import ctypes
    _libc = ctypes.CDLL("libc.so.6")
    _libc.mallopt(-8, 2)        # M_ARENA_MAX = 2
    _libc.mallopt(-7, 131072)   # M_TRIM_THRESHOLD = 128KB
except Exception:
    pass
...
# Источники: Habr, VC и др. публикуют "топ-5 инструментов", "обзор сервисов" и т.п.
_AD_PATTERNS = {
    # Обзоры и подборки (часто нативная реклама)
    r"топ[- ]?\d+",
    ...
    # Прямая реклама маркетплейсов / промокодов
    r"промокод",
    r"реферальн",
...
def _is_advertorial(title: str) -> bool:
    ...
    Такие статьи часто публикуются на Habr, VC и др. платформах.
    """
...
def filter_article(article: Dict[str, Any], user_prefs: Optional[Dict[str, Any]] = None) -> bool:
    ...
    # Фильтр рекламных/обзорных статей (native ad)
    ...
    try:
        if config.WEBHOOK_URL:
            ...
            await bot.set_webhook(
                url=f"{config.WEBHOOK_URL}{config.WEBHOOK_PATH}", drop_pending_updates=True
            )
