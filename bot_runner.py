...
async def job_collect_news() -> None:
    ...
    try:
        ...
        MAX_QUEUE_MINUTES = 15  # максимум 15 минут очереди (весь интервал сбора)
        scheduled_count = 0
        skipped_yellow = 0
        for article in filtered_news:
            link = article.get("link")
            ...
            # Yellow публикуем с задержкой 1 час (или пропускаем если очередь > 15 мин)
            if level == "yellow":
                queue_minutes = (run_time - datetime.now()).total_seconds() / 60
                if queue_minutes > 15:
                    logger.info(f"⏭ Yellow пропущен (очередь {queue_minutes:.0f} мин): {title}...")
                    skipped_yellow += 1
                    continue
        ...
        logger.info(f"✅ В очередь добавлено: {scheduled_count}, пропущено (yellow): {skipped_yellow}")
