...
- Вопросы в конце комментария ("Обсуждаем?", "Согласны?", "Как считаете?", "Как думаете?", "А вы?", "Что думаете?", "Ваше мнение?", "Пишите в комментариях", "Делитесь мнением")

КРИТИЧЕСКИ ВАЖНО — ТОЛЬКО ФАКТЫ ИЗ НОВОСТИ:
- Используй ТОЛЬКО имена, даты и факты, которые есть в предоставленном заголовке и тексте новости.
- НЕ придумывай имена, должности, даты, события или цифры, которых нет в тексте.
- Если не уверен в детали — не упоминай её. Лучше сказать меньше, но точно.
- Если в новости упоминается политик, используй ТОЛЬКО то имя, которое дано в тексте. НЕ подменяй на других людей.
- Комментарий должен быть основан ТОЛЬКО на предоставленном заголовке и summary, а не на внешних знаниях.
...
Текст: {summary[:900]}"""
    ...
    def _validate_output(self, text: str) -> str:
        ...
        # Убираем engagement-вопросы (FEAT-007)
        engagement_phrases = [
            "Обсуждаем?",
            "Согласны?",
            "Как считаете?",
