...
from utils.image_search import _extract_top_keywords, _build_image_query, _pick_best_image
from utils.deduplicator import deduplicate_articles, _title_similarity
from bot_runner import filter_article, _is_junk
...
def test_build_image_query_short():
    ...
    summary = "Комиссия по ценным бумагам США приняла решение, которое повлияло на рынок криптовалют."
...
def test_is_junk_allows_real_news():
    ...
    real_articles = [
        {"title": "Трамп заявил о сделке с Ираном", "summary": "Президент США сообщил о прогрессе в переговорах с Тегераном по ядерной программе."},
        {"title": "Bitcoin вырос на 5%", "summary": "Крупнейшая криптовалюта обновила максимум после заявлений ФРС."},
    ]
...
def test_deduplication_threshold():
    ...
    articles = [
        {"title": "Трамп встретился с Путиным в Москве", "link": "http://example.com/1", "score": 8, "published": now},
        {"title": "Путин и Трамп провели переговоры в Москве", "link": "http://example.com/2", "score": 7, "published": now - timedelta(minutes=5)},
        {"title": "Bitcoin вырос до нового рекорда", "link": "http://example.com/3", "score": 6, "published": now - timedelta(minutes=10)},
    ]
    ...
    assert len(result) == 2, f"Ожидали 2 статьи, получили {len(result)}: {[r['title'] for r in result]}"
...
def test_politician_check_in_image_search():
    ...
    results = [
        {"url": "https://ichef.bbci.co.uk/news/800/trump-speech-2024.jpg", "title": "Trump at White House press conference", "width": 800, "height": 600},
        {"url": "https://cdn.cnn.com/iran-map-2024.jpg", "title": "Map of Iran region", "width": 800, "height": 600},
    ]
